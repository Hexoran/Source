...
exports.leaveTeamTour = function (room, user) {
	...
	if (wars[roomId].tourRound !== 0) {
		...
		Rooms('warrooma').add ('|raw' + '<b>' + user + '</b> se ha autodescalificado de la guerra.');
...
exports.startTeamTour = function (room) {
	...
	if (wars[roomId].type === 'lineups') {
		...
		var teamAMembers = Object.assign([], exports.getAvailableMembers(wars[roomId].teamAMembers));;
		var teamBMembers = Object.assign([], exports.getAvailableMembers(wars[roomId].teamBMembers));;
	}
	var memberCount = Tools.shuffle(teamAMembers, teamBMembers);;
	var matchups = {};
...
exports.newRound = function (room) {
	...
	Rooms('warrooma').add('|raw|' + exports.viewTourStatus(roomId));
...
exports.autoEnd = function (room) {
	...
	Rooms('warrooma').add('|raw|' + exports.viewTourStatus(roomId)+ htmlEndTour);
	...
	war: function(target, room, user, connection) {
		...
		switch (toId(params[0])) {
			...
			case 'create':
				...
				if (size < 1) return this.sendReply("Mínimo deben ser 3 jugadores por clan.");
				...
				switch (toId(params[1])) {
					case 'standard':
						...
						Rooms('warrooma').add('|raw|' + '<hr /><h2><font color="green">' + user.name + ' ha iniciado una guerra standard en formato ' + format + ' entre ' + Chat.escapeHTML(params[4]) + " y " + Chat.escapeHTML(params[5]) +  '.</font></h2><b>Para unirse a la war: <button name="send" value="/war join">/war join</button></b><br /><b><font color="blueviolet">Jugadores por equipo:</font></b> ' + size + '<br /><font color="blue"><b>FORMATO:</b></font> ' + format + '<hr /><br /><font color="red"><b>Recuerda que debes mantener tu nombre durante toda la duración de la guerra.</b></font>');
						Rooms('lobby').add("|raw| <div style=\"background: black; color: #F6CEEC; border: 3px solid #CC2952; padding: 8px;\"><center>Ha iniciado una guerra standard entre los Clanes " + toId(params[4]) + " y " + toId(params[5]) + " en formato " + format + "<br><br><center><button name=\"send\" value=\"/join warrooma\" style=\"border: 1px solid #000 ; background: #CC2952; box-shadow: 0px 1px 1px rgba(255 , 255 , 255 , 0.3) inset ; color: #000 ; border-radius: 4px ; margin: 1px 0px ; padding: 3px 4px\"><strong>Observar</strong></button></div>");
						break;
					case 'total':
			...
			case 'delete':
				...
				Rooms('warrooma').add('|raw|' + '<hr /><center><h2><font color="green">' + user.name + ' ha cancelado la guerra entre ' + tourData.teamA + " y " + tourData.teamB + '.</h2></font></center><hr />');
			...
			case 'join':
				...
				if (freePlaces > 0) {
					Rooms('warrooma').add('|raw|' + '<b>' + user.name + '</b> se ha unido a la guerra. Quedan ' + freePlaces + ' plazas.');
				} else {
					Rooms('warrooma').add('|raw|' + '<b>' + user.name + '</b> se ha unido a la guerra. Comienza la guerra!');
					War.startTeamTour(roomId);
					Rooms('warrooma').add('|raw|' + War.viewTourStatus(roomId));
				}
				break;
			case 'l':
			case 'salir':
			case 'leave':
				...
				if (err) return this.sendReply(err);
				var freePlaces =  War.getFreePlaces(roomId);
				Rooms('warrooma').add('|raw|' + '<b>' + user.name + '</b> ha salido de la guerra. Quedan ' + freePlaces + ' plazas.');
				break;
			case 'auth':
			...
			case 'reg':
				...
				if (err) return this.sendReply(err);
				if (toId(user.name) === toId(tourData.authA)) Rooms('warrooma').add('|raw|' + user.name + ' ha registrado la alinación para ' + tourData.teamA + '.');
				if (toId(user.name) === toId(tourData.authB)) Rooms('warrooma').add('|raw|' + user.name + ' ha registrado la alinación para ' + tourData.teamB + '.');
				break;
			case 'empezar':
			case 'begin':
			case 'start':
			...
			case 'size':
				...
				var freePlaces =  War.getFreePlaces(roomId);
				if (freePlaces > 0) {
				Rooms('warrooma').add('|raw|' +'<b>' + user.name + '</b> ha cambiado el tama&ntilde;o de la guerra a ' + parseInt(params[1]) + '. Quedan ' + freePlaces + ' plazas.');
				} else {
					Rooms('warrooma').add('|raw|' + '<b>' + user.name + '</b> ha cambiado el tama&ntilde;o de la guerra a ' + parseInt(params[1]) + '. Comienza la guerra!');
					War.startTeamTour(roomId);
					Rooms('warrooma').add('|raw|' + War.viewTourStatus(roomId));
				}
				break;
			...
			case 'dq':
				...
				if (!canReplace && !this.can('games', room)) return false;
				var tourData = War.getTourData(roomId);
				if (!tourData) return this.sendReply("No había ninguna guerra en esta sala");
...
Rooms.global.startBattle = function(p1, p2, format, rated, p1team, p2team) {
	...
	if (matchup) {
		...
		Rooms('warrooma').add('|raw|' + "<a href=\"/" + newRoom.id + "\" class=\"ilink\"><b>La batalla entre " + p1.name + " y " + p2.name + " ha comenzado.</b></a>");
...
Rooms.BattleRoom.prototype.win = function(winner) {
	...
	if (this.war) {
		...
		if (matchup) {
			...
			if (!losser) {
				//tie
			Rooms('warrooma').add('|raw|' + 'La batalla entre <b>' + this.p1.name + '</b> y ' + this.p2.name + '</b> ha terminado en empate. Inicien otra batalla.');
				War.invalidate(matchup.tourId, matchup.matchupId);
			} else {
			Rooms('warrooma').add('|raw|' + '<b>' + winner + '</b> ha ganado su batalla contra ' + losser + '.</b>');

...
exports.commands = {
	...
	medallaliga: function (target, room, user) {
		...
		if (!medalData) return this.sendReply("La medalla especificada no existe.");
		return this.sendReplyBox(userT + ' puede hacer entrega de: <b>' + Chat.escapeHTML(medalData.name) + ':</b><br /><br /><img src="' + encodeURI(medalData.image) + '" title="' + Chat.escapeHTML(medalData.name) + '" width="' + Chat.escapeHTML(medalData.width) + '" height="' + Tools.escapeHTML(medalData.height) + '" />&nbsp;');
	},

...
class PassTheBomb {
	...
	nextRound() {
		...
		this.release = setTimeout(() => {
			...
			this.room.add('|uhtmlchange|' + this.getMsg() + '<br><b style = "font-size: 10pt;">The bomb has been passed to <span style = "color:' + Equ.Color(this.holder) + '">' + Chat.escapeHTML(player) + '</span>!</b></div>').update();
			this.canPass = true;
			this.resetTimer();
	...
	pass(user, target, self) {
		...
		this.room.add('|html|' + user.name + ' passed the bomb to <b style = "' + Equ.Color(targetId) + '">' + this.players.get(targetId).name + '</b>!');
	...
	resetTimer() {
		this.timer = setTimeout(() => {
			let player = this.players.get(this.holder).name;
			this.room.add('|html|<b>The bomb exploded and killed <span style = "' + Equ.Color(this.holder) + '">' + player + '</span>').update();
	...
	getWinner() {
		...
		let msg = '|html|<div class = "infobox"><center>The winner of this game of Pass the Bomb is <b style = "color:' + Equ.Color(winner) + '">' + Chat.escapeHTML(winner) + '!</b> Congratulations!</center>';

...
exports.commands = {
	...
	badges: function (target, room, user) {
		...
		case 'set':
			...
			if (Users.get(targetUser)) Users.get(userid).popup('|modal||html|You have received a badge from ' + Equ.Color(toId(user), true) + ': <img src="' + Db.badgeData.get(selectedBadge)[1] + '" width="16" height="16"> (' + selectedBadge + ')');
		...
		case 'take':
			...
			Users.get(userid).popup('|modal||html|' + Equ.Color(user.name, true) + ' has taken the ' + selectedBadge + ' from you. <img src="' + Db.badgeData.get(selectedBadge)[1] + '" width="16" height="16">');
			break;
		case 'delete':
			if (!this.can('ban')) return false;
